...
import { useCuentaStore } from "@/app/lib/useCuentaStore"; // ✅ import agregado
...
export default function CrearCuentaForm() {
  ...
  const { setCuentaActiva } = useCuentaStore(); // ✅ movido aquí dentro del componente
  ...
  const onSubmit: SubmitHandler<CreateCuentaInput> = (data) => {
    startTransition(async () => {
      ...
      if (result.ok && result.data) {
        ...
        const cuenta = result.data as CuentaCreadaData;
        setCuentaCreada(cuenta);

        // ✅ Guardamos en el store para que /transactions lo use
        setCuentaActiva({
